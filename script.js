document.addEventListener('DOMContentLoaded', () => {
  ...
  const showRemovals = document.getElementById('showRemovals');
  const positiveChart = document.getElementById('positiveChart');
  const removalChartBox = document.getElementById('removalChartBox');

  let removalChartInitialized = false;

  const toggleChart = (target) => {
    [positiveChart, removalChartBox].forEach(div => div.style.display = 'none');
    [showPositive, showRemovals].forEach(btn => btn.classList.remove('active'));

    if (target === 'positive') {
      positiveChart.style.display = 'block';
      showPositive.classList.add('active');
    } else if (target === 'removal') {
      removalChartBox.style.display = 'block';
      showRemovals.classList.add('active');

      if (!removalChartInitialized) {
        setTimeout(() => {
          initRemovalChart();
          removalChartInitialized = true;
        }, 100);
      }
    }
  };

  showPositive.addEventListener('click', () => toggleChart('positive'));
  showRemovals.addEventListener('click', () => toggleChart('removal'));

  // Positive Reviews Line Chart
  ...
  // Negative Reviews: Received vs Removed
  function initRemovalChart() {
    ...
    const gradRec = ctx.createLinearGradient(0, 0, 0, 400);
    gradRec.addColorStop(0, 'rgba(231, 76, 60, 0.4)');
    gradRec.addColorStop(1, 'rgba(231, 76, 60, 0)');

    const gradRem = ctx.createLinearGradient(0, 0, 0, 400);
